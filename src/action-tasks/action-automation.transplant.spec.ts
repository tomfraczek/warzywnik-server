...
describe('ActionAutomationService transplant routines', () => {
  const service = new ActionAutomationService({} as EntityManager);

  const makePlanting = (transplantedAt: Date | null): Planting =>
    ({
      id: 'planting-1',
  ...
  it('generates AFTER_TRANSPLANT_DAYS routine tasks only when transplantedAt is present', () => {
    ...
    const withoutTransplantedAt = target.buildCandidatesForPlanting(
      makePlanting(null),
      [rule],
    );
    expect(withoutTransplantedAt).toHaveLength(0);

    const transplantedAtForTodayDue = addDays(
      normalizeDueAt(new Date(), 'Europe/Warsaw'),
      -1,
    );
    const withTransplantedAt = target.buildCandidatesForPlanting(
      makePlanting(transplantedAtForTodayDue),
      [rule],
    );
    expect(withTransplantedAt.length).toBeGreaterThan(0);
    expect(withTransplantedAt[0]?.trigger).toBe(
      ActionRuleTrigger.AFTER_TRANSPLANT_DAYS,
    );
    expect(
      normalizeDueAt(withTransplantedAt[0].dueAt, 'Europe/Warsaw').getTime(),

...
describe('Scenario F — delivery policy resolution for known intent keys', () => {
  it('WATERING_TODAY → PUSH_DIGEST', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { NotificationAggregatorService } = require('./notification-aggregator.service') as {
      NotificationAggregatorService: new (...args: unknown[]) => Record<string, (...args: unknown[]) => unknown>;
    };
    ...
    const result = (instance as Record<string, (...args: unknown[]) => unknown>)[
  ...
  it('HIGH priority garden risk → PUSH_IMMEDIATE', () => {
    ...
    const result = (instance as Record<string, (...args: unknown[]) => unknown>)[
      'resolveGardenRiskDeliveryPolicy'
